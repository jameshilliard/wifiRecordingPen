...
int WebRtcVad_InitCore(VadInstT* self) {
  ...
  //zss++ WebRtcSpl_ResetResample48khzTo8khz(&self->state_48_to_8);
...
int WebRtcVad_CalcVad48khz(VadInstT* inst, const int16_t* speech_frame,
  ...
  //zss++ int32_t tmp_mem[480 + 256] = { 0 };
  const size_t kFrameLen10ms48khz = 480;
  //zss++ const size_t kFrameLen10ms8khz = 80;
  size_t num_10ms_frames = frame_length / kFrameLen10ms48khz;

  for (i = 0; i < num_10ms_frames; i++) {
    //zss++ WebRtcSpl_Resample48khzTo8khz(speech_frame,
    //zss++                               &speech_nb[i * kFrameLen10ms8khz],
    //zss++                               &inst->state_48_to_8,
    //zss++                               tmp_mem);
  }
